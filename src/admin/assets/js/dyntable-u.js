/*  DynTable, developed by Nacho Brito based on previous work by Adrew Plummer.
 *  Copyright (c) 2011 Nacho Brito
 *
...
var DynTable = new Class({
    ...
    initialize: function(id, options){
        ...
        if(this.options.addNewRows && this.form){            
            ...
            this.addRow = new Element("p", {
                "class": "addRow"
            });
            this.addRow.adopt(new Element("a", {
                html: this.options.addRowLabel, 
                events: {
    ...
    onComplete: function(response){
        ...
        if(!json || json.affected <= 0){
            ...
            if(json.action == "insert" || json.action == "edit"){                 
                var ord = this.currentJob.row.getElement('.ord');
                if(ord) ord.set("text", json.ord);

                var formElements = this.currentJob.row.getElements("select,input,textarea,span.attach,span.pac");
                ...
                if(json.auto){
                    var idAI = this.currentJob.row.getElement('.auto_increment');
                    if(idAI) el.set("text", json.key);
                }
                ...
                if(json.error){
                    this.currentJob.row.addClass('invalid');
                }
                else{
                    this.currentJob.row.removeClass('invalid');
                }
                var msg = json.message?json.message:'';
                this.currentJob.row.getElement('.dyntable_messages').set('html','<p>' + msg + '</p>');
            }
    ...
    addNewRow: function(event){
        ...
        //Aplicamos máscara para fechas:
        newDataRow.getElements('input.mask-date').each(function(item){            
            item.meiomask('fixed.date');
        });
        newDataRow.erase("style");
        newDataRow.inject(this.tbody);
        if(window.HotText) new HotText(".hotText", {
            scope: newDataRow
        });
    ...
    removeRow: function(row){
        ...
        if(this.rows.length < 1){
            ...
            var noData = new Element("td", {
                "text": this.options.noDataMessage,
                "colspan": colspan,
                "align": "center"
            });
            var noDataRow = new Element("tr", {
                "class": "noDataRow odd"
            });
