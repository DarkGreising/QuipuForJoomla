...
?>
<form action="<?php echo JRoute::_('index.php?option=com_quipu&view=purchaseorders'); ?>" method="post" name="adminForm" id="adminForm">
    <fieldset id="filter-bar">
        ...
        <div class="filter-select fltrt">
            <select name="filter_supplier" class="inputbox <?php echo $this->state->get('filter.supplier','*')!='*'?'filter-on':''?>" onchange="this.form.submit()">
                <option value="*"><?php echo JText::_('COM_QUIPU_FILTER_BY_SUPPLIER');?></option>
                <?php foreach($this->suppliers as $id=>$customer):?>
                <option value="<?php echo $customer->id?>" <?php echo ($customer->id==$this->state->get('filter.supplier'))?'selected="true"':''?>><?php echo $customer->name?></option>
                <?php endforeach;?>
            </select>
        </div>
        <div class="filter-select fltrt">
